            ...
            while (DateTime.UtcNow < deadline && !ct.IsCancellationRequested)
            {
                var raw = await webView.CoreWebView2.ExecuteScriptAsync(@"
                    (function() {
                        ...
                        // Pass 1: find 'X% used' that lives inside a 'Current session' ancestor
                        ...
                        // Fallback: first leaf 'X% used' anywhere
                        if (pct === null) {
                        ...
                        // Reset time: 'Resets Mon 2:59 PM' or 'Resets in X hr Y min' etc.
